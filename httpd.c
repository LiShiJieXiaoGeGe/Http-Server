				 ...
				 const char *method, const char *query_string)
{
	...
	if (pid == 0) /* 子进程: 运行CGI 脚本 */
	{
		...
		dup2(cgi_output[1], 1); // 写到标准输出
		dup2(cgi_input[0], 0);	// 重定向到标准输入 从标准输入读

		close(cgi_output[0]); //关闭了cgi_output中的读通道
		close(cgi_input[1]);  //关闭了cgi_input中的写通道

		sprintf(meth_env, "REQUEST_METHOD=%s", method);
		putenv(meth_env); // 输出到环境变量
...
int startup(u_short *port)
{
	...
	if (*port == 0)		   /*动态分配一个端口 */
	{
		...
		*port = ntohs(name.sin_port); // 获取动态分配的端口
	}
...
{
	int server_sock = -1;
	u_short port = 80; //默认监听端口号 port 为80
	int client_sock = -1;
